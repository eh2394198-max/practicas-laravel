                ...
                <article class="flex flex-col bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 overflow-hidden relative">
                    ...
                    @hasanyrole('admin|editor')
                        <div class="absolute top-2 right-2 flex gap-2 z-10">
                            {{-- CORREGIDO: Se eliminó 'admin.' --}}
                            <a href="{{ route('posts.edit', $post) }}" class="bg-yellow-400 text-white p-2 rounded-full shadow hover:bg-yellow-500 transition" title="Editar">
                            ...
                            @role('admin')
                                {{-- CORREGIDO: Se eliminó 'admin.' --}}
                                <form action="{{ route('posts.destroy', $post) }}" method="POST" onsubmit="return confirm('¿Borrar este post?')">
                                    @csrf @method('DELETE')
